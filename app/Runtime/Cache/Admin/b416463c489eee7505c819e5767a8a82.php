		...
		<div class="main-content-inner">
			<div id="breadcrumbs" class="breadcrumbs">
				<ul class="breadcrumb">
					...
					<li>
						<a href="#"><?php echo L('comment_manage');?></a>
					</li>
					<li class="active"><?php echo L('comment_list');?></li>
				</ul>
							...
							<div class="col-xs-12">
								...
								<table class="table table-striped table-bordered table-hover" id="comment_list">
										...
										<tr>
											...
											<th class="center" width="50%"><?php echo L('comment_content');?></th>
											<th class="center" width="20%"><?php echo L('comment_user');?></th>
											<th class="center hidden-480"><?php echo L('from');?></th>
											<th class="center">操作</th>
										</tr>
										<?php if(is_array($comment)): foreach($comment as $key=>$vo): ?><tr>
											<td class="center">
												<label class="position-relative"><input type="checkbox" name="comment_id[]" value="<?php echo ($vo["id"]); ?>" class="ace"><span class="lbl"></span></label>
											</td>
											<td><?php echo ($vo["content"]); ?></td>
											<td><?php echo ($vo["user_id"]); ?></td>
											<td><?php echo ($vo["article_id"]); ?></td>
											<td><a href='javascript:' id='bootbox-regular' onclick="huifu(<?php echo ($vo["id"]); ?>)">回复</a></td>
		...
		function commentact(act){
			...
			if(act =='del'){
				if(confirm('确定删除该评论?')){
					...
					$.post('<?php echo U('comment/up');?>',{id:checkedList.toString(),type:'del'},function(data){
						if(!data.status){
			...
			if(act =='hide'){
				$.post('<?php echo U('comment/up');?>',{id:checkedList.toString(),type:'hide'},function(data){
						if(!data.status){
							op_error(data.info,'温馨提示');
			...
			if(act =='pub'){
				$.post('<?php echo U('comment/up');?>',{id:checkedList.toString(),type:'pub'},function(data){
						if(!data.status){
							op_error(data.info,'温馨提示');
		...
		function huifu(id){
			...
			bootbox.prompt("回复",function(result){
				if(result === null){
					...
					$.post('<?php echo U('comment/reply');?>',{id:id,reply:result},function(data){
						if(data.status){
							op_success(data.info,'温馨提示');
						}else{
							op_error(data.info,'温馨提示');
